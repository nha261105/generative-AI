...
def render_sidebar(
    ...
    # Thêm params cho index status và graph stats
    index_status: str = "none",   # "cached" | "processing" | "none"
    chunk_count: int = 0,
    ...
    with st.sidebar:
        ...
        icon, label = _STATUS_MAP.get(index_status, ("⭕", "Chưa có dữ liệu"))
        stats_parts = []
        if chunk_count > 0:
            stats_parts.append(f"{chunk_count} chunks")
        gs = graph_stats or {}
        if gs.get("nodes", 0) > 0:
            stats_parts.append(f"{gs['nodes']} nodes · {gs.get('edges', 0)} edges")
        stats_str = f" · {' · '.join(stats_parts)}" if stats_parts else ""
        st.markdown(
            f'<div class="index-badge index-{index_status}">'
            f'{icon} {label}{stats_str}'
            f'</div>',
        ...
        with st.expander("⚙️ Advanced Settings", expanded=True):
            ...
            out["show_graph"] = st.toggle(
                "🌐 View Connections", value=False, key="toggle_graph"
            )

            # ── Search mode (radio) ──
            st.markdown('<div class="sidebar-section-label">Search Strategy</div>', unsafe_allow_html=True)
            mode_idx = 0 if search_mode == "semantic" else 1
            sel_mode = st.radio(
                "Search mode", ["Semantic Search (Vector)", "Ensemble Retriever (Hybrid: Vector + BM25)"],
            ...
            st.markdown('<div class="sidebar-section-label">Post-retrieval Pipeline</div>', unsafe_allow_html=True)
            out["use_rerank"] = st.toggle("Re-ranking với Cross-Encoder", value=use_rerank, key="toggle_rerank")
            out["use_selfrag"] = st.toggle("Advanced RAG với Self-RAG", value=use_selfrag, key="toggle_selfrag")

            # ── Chunk config ──
            st.markdown('<div class="sidebar-section-label">Cải thiện Chunk Strategy</div>', unsafe_allow_html=True)
            out["cs"] = st.slider("Chunk size", 500, 2000, chunk_size, 100)
            out["co"] = st.slider("Chunk overlap", 50, 300, chunk_overlap, 50)

            # ── Clear Vector Store ──
            st.markdown('<div class="sidebar-section-label">Vector Store Management</div>', unsafe_allow_html=True)
            if not st.session_state.confirm_clear_vector:
                if st.button("Clear Vector Store", key="btn_clr_vec", use_container_width=True):
        ...
        if not st.session_state.confirm_clear_history:
            if st.button("Clear History", key="btn_clr_hist", use_container_width=True):
...
def render_status_bar(vector_db, processing_step: int, file_count: int = 0):
    ...
    if processing_step == 2:
        dot, label = "processing", "Đang xử lý tài liệu..."
